...
##### `BeanDefinitionRegistryPostProcessor`执行流程
...
##### 1、可不可以在`BeanFactoryPostProcessor`去创建一个Bean，这样有什么问题？

从技术上来说这样是可以的，但是正常情况下我们不该这样做，这是因为可能会存在该执行的Bean工厂的后置处理器的逻辑没有被应用到这个Bean上。

##### 2、`BeanFactoryPostProcessor`可以被配置为懒加载吗？
